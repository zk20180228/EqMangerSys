...
//			var url = $(currTab.panel('options').content).attr('src');
					...
					success : function(data) {
						...
						if (menuList && menuList.length) {
							...
							if (index > 0) {
								$('#navDiv')
										.append(
												"<ul id=\"topnav\"><li class=\"first\">&nbsp;</li><li class=\"nav-left-arrow\"><a href=\"javascript:loadMenu(0);\"><<</a></li>");
							} else {
								$('#navDiv')
										.append(
	...
	function megaHoverOver() {
		...
		if ($(this).find(".row").length > 0) { //If row exists...
			...
			var winW = $("body").width();
			var left = (index - 1) * 120 + 30;
			if (left + rowWidth > winW) {
				$(this).find(".sub").css("margin-left",
						(winW - rowWidth - left) + "px");
			}
